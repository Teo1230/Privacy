...
class FinetunedCasualLM(LLMBase):
    ...
    def get_last_token_embedding(self, text):
        """
    Get the embedding of the last token after processing the input text.

    Parameters:
    - text (str): The input text to query the model.

    Returns:
    - last_token_embedding: The embedding of the last token.
        """
    ...
    # Get the output from the model
        with torch.no_grad():
            output = self._lm(input_ids=input_ids, output_hidden_states=True)  # Enable hidden states

    # Extract the last hidden state (the output of the last layer)
    ...
    # Get the embedding for the last token
        last_token_embedding = last_hidden_state[:, -1, :].squeeze(0)  # shape: (hidden_size,)

    # Detach and convert to NumPy
        return last_token_embedding.detach().cpu().numpy()  # Detach before converting to numpy
    def get_all_layers_embeddings_with_cache(self, text, cache_path='embedding_cache.pkl'):
        """
    Get the embeddings of all tokens across all layers after processing the input text.
    If embeddings are cached, load from cache; otherwise, compute and cache them.

    Parameters:
    - text (str): The input text to query the model.
    - cache_path (str): The path to the cache file where embeddings are stored.

    Returns:
    - all_layer_embeddings: A NumPy array of shape (num_layers, sequence_length, hidden_size)
                            containing embeddings for all tokens across all layers.
        """
    # Check if the cache exists
        if os.path.exists(cache_path):
            with open(cache_path, 'rb') as f:
                all_layer_embeddings = pickle.load(f)
        ...
        # Extract all hidden states (one per layer)
            all_hidden_states = torch.stack(output.hidden_states)  # shape: (num_layers, batch_size, sequence_length, hidden_size)

        # Remove the batch dimension as we only have one input
            all_layer_embeddings = all_hidden_states.squeeze(1)  # shape: (num_layers, sequence_length, hidden_size)

        # Convert to NumPy before caching and returning
            all_layer_embeddings = all_layer_embeddings.detach().cpu().numpy()

        # Cache the embeddings
            with open(cache_path, 'wb') as f:
                pickle.dump(all_layer_embeddings, f)
            print("Computed and saved embeddings to cache.")
